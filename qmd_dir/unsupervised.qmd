...
## {{< fa cogs >}} Feature Selection and Normalization
...
**StandardScaler (Z-score normalization)** transforms each feature to have:
- Mean (μ) = 0
...
## {{< fa shield-halved >}} Regularization in Clustering
...
**1. Sample Size Regularization**
- We use 100,000 samples instead of the full 11M+ dataset
- This prevents overfitting to noise in the data
- Ensures clusters represent genuine patterns, not random variations

**2. Feature Selection**
- We carefully select 4 meaningful features instead of using all available columns
- This reduces dimensionality and prevents the "curse of dimensionality"
- Focuses the algorithm on relevant patterns
...
- Prevents overfitting by choosing the optimal number of clusters
- Balances model complexity (number of clusters) with performance (inertia)
- Avoids creating too many clusters that fit noise rather than patterns

::: {.callout-note}
## Regularization vs Overfitting
In clustering, overfitting occurs when:
- Too many clusters are created (high K)
- Clusters fit noise rather than genuine patterns
- The model doesn't generalize to new data

Our regularization approach ensures clusters are:
...
<style>
.kc5-q{margin-bottom:16px}.kc5-q p{font-weight:700;margin:0 0 8px;font-size:.95rem}
.kc5-opts label{display:block;padding:7px 12px;margin:3px 0;background:#fff;border:1.5px solid #d0d7de;border-radius:6px;cursor:pointer;font-size:.9rem}
.kc5-opts label:hover{border-color:#0b3d91}
.kc5-opts input{margin-right:8px}
.kc5-btn{margin-top:12px;background:#0b3d91;color:#fff;border:none;border-radius:6px;padding:9px 22px;cursor:pointer;font-size:.9rem;font-weight:600}
.kc5-btn:hover{background:#093080}
.kc5-score-box{margin-top:12px;padding:12px 16px;border-radius:6px;font-weight:600;font-size:.95rem;display:none}
</style>
<h4 style="margin:0 0 16px;color:#0b3d91;font-size:1.1rem">🎯 Knowledge Check: Unsupervised Learning</h4>
<form id="kcf5">
<div class="kc5-q"><p>1. What is the key difference between supervised and unsupervised learning?</p><div class="kc5-opts">
<label><input type="radio" name="q1" value="a"> Supervised learning uses Python; unsupervised uses R</label>
<label><input type="radio" name="q1" value="b"> Supervised learning uses labeled data with a target variable; unsupervised finds patterns without labels</label>
<label><input type="radio" name="q1" value="c"> Unsupervised learning is always faster</label>
<label><input type="radio" name="q1" value="d"> Supervised learning only works on images</label>
</div></div>
<div class="kc5-q"><p>2. What algorithm is used in this lab for clustering NYC taxi trips?</p><div class="kc5-opts">
<label><input type="radio" name="q2" value="a"> K-Nearest Neighbors (KNN)</label>
<label><input type="radio" name="q2" value="b"> Linear Regression</label>
<label><input type="radio" name="q2" value="c"> MiniBatchKMeans</label>
<label><input type="radio" name="q2" value="d"> Random Forest</label>
</div></div>
<div class="kc5-q"><p>3. What method is used to determine the optimal number of clusters (K)?</p><div class="kc5-opts">
<label><input type="radio" name="q3" value="a"> Trial and error</label>
<label><input type="radio" name="q3" value="b"> The Elbow Method (plotting inertia vs K)</label>
<label><input type="radio" name="q3" value="c"> The R² score</label>
<label><input type="radio" name="q3" value="d"> Cross-validation</label>
</div></div>
<div class="kc5-q"><p>4. What was the key finding of the clustering analysis in this lab?</p><div class="kc5-opts">
<label><input type="radio" name="q4" value="a"> All taxi trips are identical</label>
<label><input type="radio" name="q4" value="b"> The model failed to find any clusters</label>
<label><input type="radio" name="q4" value="c"> Clear separation between airport/long-distance trips and urban trips</label>
<label><input type="radio" name="q4" value="d"> Passenger count is the most important feature</label>
</div></div>
<button type="button" class="kc5-btn" onclick="gradeKC5()">Check Answers</button>
</form>
<div class="kc5-score-box" id="kcs5"></div>
</div>
<script>
function gradeKC5(){
  var ans={q1:'b',q2:'c',q3:'b',q4:'c'},score=0,total=4;
  var form=document.getElementById('kcf5');
  ...
  var box=document.getElementById('kcs5');box.style.display='block';
